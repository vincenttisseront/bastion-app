...
#   CORE_REALM_SLUG         (default: default)
set -euo pipefail
...
CORE_REALM_SLUG="${PORTAL_DEFAULT_REALM_SLUG:-${SSO_PORTAL_DEFAULT_REALM_SLUG:-default}}"
...
# ── Core realm : cfg depuis DB export → volume oauth2-proxy-core ──
...
if command -v docker >/dev/null 2>&1; then
  ...
  # watch-exports-reload.sh also syncs, but apply-infra remains the explicit path).
  if ! docker compose exec -T nginx sh -s <<'EOS'
set -euo pipefail
if [ -x /sync-exports-to-confd.sh ]; then
  /sync-exports-to-confd.sh
else
  # Fallback for images without sync-exports-to-confd.sh
