...
def _ctl_action_for_exclusion(ex: WafExclusion, rule_id: int) -> str:
    ...
    # Fallback: treat as full rule remove rather than emit invalid ctl
    return f"ctl:ruleRemoveById={rule_id}"


def _drop_unsafe_secrules(lines: list[str]) -> list[str]:
    """Drop SecRules that still contain ``%`` / ``\\x25`` (macro / decode traps)."""
    out: list[str] = []
    skipping_cont = False
    for line in lines:
        if skipping_cont:
        ...
        if not stripped or stripped.startswith("#"):
            out.append(line)
            continue
        # Bare % or hex that ModSecurity expands to % both break nginx -t.
        if "%" in line or r"\x25" in line or r"\X25" in line:
            out.append(
                "# skipped generated SecRule: percent / \\x25 breaks ModSecurity parse"
            )
            skipping_cont = line.rstrip().endswith("\\")
            continue
        out.append(line)
...
def render_exclusions_generated(exclusions: list[WafExclusion]) -> str:
    ...
    for ex in exclusions:
        ...
        if uri_raw and "%" in uri_raw:
            ...
            if decoded is None:
                lines.append(
                    "# skip SecRule: URI still contains % after decode "
                    "(ModSecurity treats % as macro — use decoded path in UI)"
                )
                lines.append("")
                continue
            lines.append(
        ...
        # Global remove — only when admin explicitly has no host/URI scope.
        ...
        if host and uri:
            ...
            lines.append(f"SecRule REQUEST_URI {_uri_operator(uri, uri_match)} \\")
            lines.append(f'  "{ctl}"')
        elif host:
            lines.append(
                f'SecRule REQUEST_HEADERS:Host "@streq {_modsec_quote(host)}" \\'
            )
            lines.append(f'  "id:{sec_id},phase:1,pass,nolog,{ctl}"')
        else:
            lines.append(f"SecRule REQUEST_URI {_uri_operator(uri, uri_match)} \\")
            lines.append(f'  "id:{sec_id},phase:1,pass,nolog,{ctl}"')
        lines.append("")
    ...
    if len(lines) <= 4:
        ...
        lines.append("")
    return "\n".join(_drop_unsafe_secrules(lines))
