...
def _nginx_upstream_var(slug: str) -> str:
    """Safe nginx variable name for a realm slug (no hyphens)."""
    safe = "".join(c if c.isalnum() else "_" for c in (slug or ""))
    return f"oauth2_realm_{safe or 'x'}"
...
def generate_nginx_realms_conf(
    ...
    *,
    ...
    """Secondary-realm oauth2 locations for nginx.

    In Docker mode, upstream hostnames are set via variables so ``nginx -t`` /
    startup does **not** require oauth2-proxy (or Keycloak behind the bastion)
    to be resolvable yet — same pattern as ``$oauth2_core_upstream``.
    """
    ...
    lines = [
        ...
        "# Docker: deferred DNS via $oauth2_realm_* (nginx must start before Keycloak/oauth2)",
        "",
    ]
    docker = settings.oauth2_proxy_network_mode == "docker"
    ...
    for realm in realms:
        ...
        if docker:
            ...
            # rewrite + variable proxy_pass → URI /oauth2/... ; DNS at request time
            lines.append(
...
def generate_nginx_apps_conf(db: Session) -> str:
    """Proxy vhosts only — sso_gate / public_proxy handled elsewhere."""
    ...
    lines = [
        ...
        "# sso_gate: catalogue-only. public_proxy: exports/nginx-public-proxy-apps.conf.",
        "",
    ]
    ...
    for app in apps:
        ...
        if mode == "public_proxy":
            lines.append(
                f"# [{app.slug}] public_proxy — see nginx-public-proxy-apps.conf"
            )
            lines.append("")
            continue
