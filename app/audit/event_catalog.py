...
_RAW_EVENTS: tuple[EventDef, ...] = (
    ...
    _e('BST-ADM-0008', 'DAILY_RECAP_SENT', 'Récapitulatif quotidien envoyé', ('configuration',),
       legacy_action='portal_settings.daily_recap_sent'),
    ...
    _e('BST-ADM-1032', 'SETUP_WIZARD_COMPLETED', 'Assistant de configuration initial terminé', ('configuration',),
       legacy_action='portal_settings.setup_wizard_completed'),
    _e('BST-ADM-2001', 'LOGIN_FORM_ANALYZED', 'Analyse de formulaire de login', ('configuration',),
       legacy_action='app.login_form.analyzed'),
    _e('BST-ADM-3001', 'DAILY_RECAP_FAILED', "Échec d'envoi du récapitulatif quotidien", ('configuration',),
       legacy_action='portal_settings.daily_recap_failed'),
    ...
    _e('BST-AUTH-0009', 'ACTIVESYNC_DEVICE_DISCOVERED', 'Nouvel appareil ActiveSync découvert', ('authentication',),
       legacy_action='activesync.device_discovered'),
    _e('BST-AUTH-1001', 'ACTIVESYNC_DEVICE_APPROVED', 'Appareil ActiveSync approuvé', ('authentication', 'iam',),
       legacy_action='activesync.device_approved'),
    _e('BST-AUTH-1002', 'ACTIVESYNC_DEVICE_REVOKED', 'Appareil ActiveSync révoqué par son propriétaire', ('authentication', 'iam',),
       legacy_action='activesync.device_revoked'),
    _e('BST-AUTH-1003', 'ACTIVESYNC_DEVICE_CONTROL_ENABLED', 'Contrôle ActiveSync par appareil activé', ('authentication', 'configuration',),
       legacy_action='activesync.device_control_enabled',
       runbook="Backfill appliqué : seuls les appareils approved passent. Vérifier le parc inventorié avant de basculer."),
    _e('BST-AUTH-1004', 'ACTIVESYNC_DEVICE_UNBLOCKED', 'Appareil ActiveSync débloqué par un administrateur', ('authentication', 'iam',),
       legacy_action='activesync.device_unblocked'),
    _e('BST-AUTH-1010', 'PROFILE_PASSWORD_CHANGED', 'Mot de passe modifié depuis le profil', ('authentication', 'iam',),
       legacy_action='profile.password_changed'),
    _e('BST-AUTH-1011', 'FORGOT_PASSWORD_REQUEST', 'Demande de réinitialisation de mot de passe', ('authentication',),
       legacy_action='auth.forgot_password',
       runbook="Vérifier l'envoi SMTP si l'utilisateur ne reçoit pas l'email."),
    ...
    _e('BST-AUTH-2006', 'ACTIVESYNC_DEVICE_DENIED', 'Appareil ActiveSync refusé (contrôle actif)', ('authentication',),
       legacy_action='activesync.device_denied',
       runbook="Contrôle par appareil actif : pending/rejected/blocked ou DeviceId absent. L'utilisateur valide dans le portail sauf blocked_by_admin."),
    _e('BST-AUTH-2007', 'ACTIVESYNC_DEVICE_REJECTED_BY_USER', 'Appareil ActiveSync refusé par son propriétaire', ('authentication', 'iam',),
       legacy_action='activesync.device_rejected',
       runbook="Signal fort : possible mot de passe volé. Vérifier les autres accès du compte."),
    _e('BST-AUTH-2008', 'ACTIVESYNC_DEVICE_BLOCKED_BY_ADMIN', 'Appareil ActiveSync bloqué par un administrateur', ('authentication', 'iam',),
       legacy_action='activesync.device_blocked',
       runbook="Vérifier le motif (vol, comportement suspect) ; l'utilisateur ne peut pas lever ce blocage."),
    _e('BST-AUTH-2009', 'ACTIVESYNC_DEVICE_UNIDENTIFIED', 'Appareil ActiveSync non identifiable', ('authentication',),
       legacy_action='activesync.device_unidentified',
       runbook="DeviceId non extractible : laissé passer si contrôle inactif ; 403 si contrôle actif. Regrouper par miss_family : 'decoder_failure' = corriger le décodeur MS-ASHTTP ; 'no_device_sent' = identifier le client avant la bascule."),
    _e('BST-AUTH-4001', 'AUTH_BYPASS_ATTEMPT', "Tentative de contournement de l'authentification", ('authentication', 'intrusion_detection',),
       legacy_action='security.identity_header_spoof',
       runbook='Isoler la route, vérifier la chaîne oauth2-auth / X-Portal-Internal-Token, auditer les accès récents.'),
    ...
    _e('BST-SESS-1006', 'PROFILE_SESSION_REVOKED', 'Session révoquée depuis le profil utilisateur', ('session', 'authentication',),
       legacy_action='profile.session_revoked'),
    ...
    _e('BST-SYS-1010', 'HOT_STORE_REALIGN_REQUESTED',
       'Réalignement du mot de passe hot-store demandé', ('configuration',),
       legacy_action='hot_store.realign_requested'),
    ...
    _e('BST-WAF-2013', 'WAF_CONFIG_APPLY_ROLLBACK', 'Apply WAF annulé (validation ou rollback)', ('intrusion_detection', 'configuration',),
       legacy_action='security.waf.apply_failed',
       runbook='Lire error / engine_wait dans les détails ; vérifier snapshot nginx-waf-snapshot.json et exports modsecurity.'),
    ...
    _e('BST-WAF-1015', 'WAF_SUBDOMAIN_PROMOTED_ON', 'Promotion moteur CRS subdomain DetectionOnly → On (+ smoke)', ('intrusion_detection', 'configuration',),
       legacy_action='security.waf.promote_subdomain_on',
       runbook='Subdomain déjà armé DetectionOnly + profil On requis. Échec → retour DetectionOnly (pas Off).'),
...
_ACTION_DOMAIN_PREFIXES: tuple[tuple[str, str], ...] = tuple(
    sorted(
        (
            ...
            ("activesync", "AUTH"),
            ("security.sso", "AUTH"),
            ("security.identity_header_spoof", "AUTH"),
