...
# Inline host lines into the map{} (do NOT use map+include — fragile with
# comments/`$`/IPv6). Only sanitize hostname keys (no colons).
MAP_SRC=/var/lib/sso-portal/exports/nginx-known-hosts.map
if [[ ! -f "$MAP_SRC" ]]; then
  {
    echo "# stub — regenerated by export_app_catalogue_files"
    echo "127.0.0.1 0;"
    echo "localhost 0;"
