...
| Transfer ↔ `/auth/login` (HAR 302/302) | Snippet `subdomain-auth` utilisait `$bastion_auth_*` vides sur la sous-requête → `X-Original-Host` omis → Host = upstream Docker → FastAPI 401 `no-app` alors que `/auth/login` accepte `bastion_session` | Snippet doit utiliser **`$host` / `$http_cookie` / `$cookie_bastion_session`** (pas de customs) ; **rebuild/reload bastion-nginx** (fichier `docker/nginx/snippets/subdomain_auth_common.conf`) ; Apply infra si l’export a changé |
