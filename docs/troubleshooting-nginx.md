...
| Transfer ↔ `/auth/login` (`ae=no-session:ck=90`) | Rebuild Cookie actif mais capture **non sticky** : un rewrite voit `$http_cookie` CrushAuth-only → `bastion_session=; CrushAuth=…` (≈90) + `x=0` | 1) Rebuild **bastion-nginx** (`$bastion_fresh_session` + `$bastion_pick_*` maps) + **bastion-app** (map sticky, **sans** `if`) 2) Apply 3) `grep bastion_pick_session` dans export + map 4) HAR : plus de `ck=90` / `x=0` |
| Transfer `404` nginx sur `/WebInterface/new-ui/index.html` (auth OK, tuile Indisponible) | Sticky via `if ($bastion_fresh_session…)` dans `location /` — nginx « if is evil » : `if` vrai saute `try_files`/`proxy_pass` → 404 HTML nginx/1.27.x | 1) Rebuild **bastion-nginx** + **bastion-app** (maps `$bastion_pick_*`, **pas** de `if` sticky) 2) Apply + reload 3) `grep -v 'if (\$bastion_fresh'` + `grep bastion_pick_session` 4) HAR : 200/302 UI, pas 404 nginx |
