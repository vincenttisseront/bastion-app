...
| Transfer ↔ `/auth/login` (`ae=no-session:ck=90`) | Rebuild Cookie actif mais capture **non sticky** : un rewrite voit `$http_cookie` CrushAuth-only → `bastion_session=; CrushAuth=…` (≈90) + `x=0` | 1) Rebuild **bastion-nginx** (`$bastion_fresh_session` map) + **bastion-app** (sticky `if ($bastion_fresh_session…)`) 2) Apply 3) `grep bastion_fresh_session` dans export + map 4) HAR : plus de `ck=90` / `x=0` |
