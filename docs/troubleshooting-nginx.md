...
| Transfer ↔ `/auth/login` (HAR 302/302) | `set $bastion_auth_*` au niveau **server{}** : la sous-requête `auth_request` réécrit ces vars à vide → 401 alors que le portail accepte `bastion_session` | Capturer Cookie/Host **dans `location /` avant `auth_request`** ; déployer image/code ; **Admin → Infrastructure → Appliquer l'infrastructure** (écrit `exports/nginx-subdomain-apps.conf`) ; rebuild/reload **bastion-nginx** (snippet + `watch-exports-reload`) |
