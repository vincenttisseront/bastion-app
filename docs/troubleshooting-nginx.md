...
| Transfer ↔ `/auth/login` (`ae=no-session:ck=72`) | Filtre CrushFTP `Cookie: CrushAuth=…` dans le **même** `location /` que `auth_request` → nginx hérite le Cookie filtré (FastAPI sans `bastion_session`, `x=0`) | 1) Rebuild **bastion-app** (≥ named `@app_upstream_*` + snippet `pass_request_headers off`) 2) Apply (re-export Transfer) 3) `grep -E 'try_files.*@app_upstream|pass_request_headers|upstream_cookie'` 4) HAR : plus de `ck=72` |
| Transfer ↔ `/auth/login` (`ae=no-session:ck=90`) | Rebuild Cookie actif mais capture **rejouée** pendant auth avec `$http_cookie` CrushAuth-only → `bastion_session=; CrushAuth=…` (≈90) + `x=0` | 1) Rebuild **bastion-app** (`return 418` → `@bastion_auth_gate_*`, snapshot `$cookie_bastion_session` une fois) + **bastion-nginx** 2) Apply 3) `grep bastion_auth_gate` 4) HAR : plus de `ck=90` / `x=0` |
| Transfer `404` nginx sur `/WebInterface/new-ui/index.html` (auth OK) | Sticky via `if ($bastion_fresh_session…)` dans `location /` — « if is evil » saute `try_files` → 404 HTML nginx | `return 418` → named gate (auth+try_files) ; **pas** de `if` sticky |
| Transfer `500` nginx sur `/WebInterface/new-ui/index.html` | Map sticky `$bastion_pick_*` lisant `$bastion_pass_*` + `set $pass $pick` = **dépendance circulaire** → 500 HTML nginx/1.27.x | 1) Rebuild **bastion-nginx** (plus de maps `pick_*`) + **bastion-app** (418 gate) 2) Apply 3) `grep -v bastion_pick` 4) HAR : 200/302 UI, pas 500 |
