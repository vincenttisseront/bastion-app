...
| Transfer ↔ `/auth/login` (`ae=no-session:ck=90`) | Rebuild Cookie rejoué pendant auth avec `$http_cookie` CrushAuth-only → `bastion_session=; CrushAuth=…` (≈90) + `x=0` | Snapshot `$cookie_bastion_session` + auth dans `location /` ; filtre CrushFTP seulement dans `@app_upstream_*` |
| Transfer `404` nginx sur `/WebInterface/new-ui/index.html` (auth OK) | Sticky `if ($bastion_fresh_session…)` saute `try_files` (if is evil) | **Pas** de `if` sticky ; `try_files` / `proxy_pass` dans `location /` |
| Transfer `500` nginx sur `/WebInterface/new-ui/index.html` | Map `$bastion_pick_*` ↔ `$bastion_pass_*` = cycle → 500 | Pas de maps `pick_*` ; snapshot `$cookie_bastion_session` |
| Transfer `401 Authorization Required` nginx brut (pas de redirect portail) | `#38` `return 418` → `@bastion_auth_gate_*` : `error_page 401` imbriqué ne déclenche pas `@portal_redirect` | Auth + `error_page 401` dans **le même** `location /` (pas de handoff 418) |
