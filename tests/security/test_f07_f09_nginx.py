...
def test_subdomain_auth_snippet_still_internal():
    ...
    # No self-referential pick maps (cycle with set $pass $pick → nginx 500).
    assert "map $bastion_fresh_session $bastion_pick_session" not in auth_map
    assert "map $bastion_fresh_session $bastion_pick_cookie" not in auth_map
    assert '""      $bastion_pass_session;' not in auth_map
    assert '""      $bastion_pass_cookie;' not in auth_map
