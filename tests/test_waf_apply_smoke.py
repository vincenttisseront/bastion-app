...
from app.models import AuditLog, WafProfile
...
def test_apply_rolls_back_when_armed_and_smoke_fails(db_session, tmp_path: Path):
    ...
    with patch(
        ...
        return_value=(True, "reload ok"),
    ) as sync_reload, patch(
    ...
    assert sync_reload.call_count >= 1
...
def test_apply_logs_failed_action_when_engine_mode_not_reached(db_session, tmp_path: Path):
    settings = _settings(tmp_path)
    db_session.add(
        WafProfile(
            name="Production",
    ...
    with patch(
        ...
        waf_service.apply_waf(db_session, settings, actor="admin")

    row = db_session.query(AuditLog).order_by(AuditLog.id.desc()).first()
    assert row is not None
    assert row.action == "security.waf.apply_failed"
    assert row.event_code == "BST-WAF-2013"
    assert row.details.get("ok") is False
